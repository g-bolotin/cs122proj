...
class Fishhead(Enemy):
    def __init__(self, center_x=0, center_y=0, scale=1):
        ...
        for i in range(1, 9):
            ...
            texture = arcade.load_texture(get_resource_path("assets/enemies/fishhead/side-walk/" + texture_name))
            self.walk_left_textures.append(texture)

        # Load walk left animation frames
        for i in range(1, 9):
            texture_name = f"fishhead-68-{i}.png"
            texture = arcade.load_texture(get_resource_path("assets/enemies/fishhead/side-walk/" + texture_name), flipped_horizontally=True)
